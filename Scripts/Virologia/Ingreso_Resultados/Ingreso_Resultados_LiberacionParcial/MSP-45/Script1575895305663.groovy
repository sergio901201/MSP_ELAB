...
String[] resultFC = crud.IngresoLP1Virologia(fechaBDD, fechaBDH, numeroSOL, estudio, muestra, longitud)

println ("El examen primero es:" + resultFC[0])
println ("El examen primero está liberado o no:" + resultFC[1])

boolean filtroF, liberado
String valSelect = ""

if(longitud>0){
	ElemTable = WebUI.executeJavaScript('return $("#span_vEST_EXA_ROTULO_0002").text();', null)
	valSelect = WebUI.executeJavaScript('return $("#vLIBERADO_0002").val();', null)
	if(valSelect == "N"){
		liberado = false
	}
	if(resultFC[0].equals(ElemTable)){
		WebUI.executeJavaScript('$("#vLIBERADO_0002").val("S"); $("#vLIBERADO_0002").change();', null)
		
		WebUI.delay(1)
		
		WebUI.click(findTestObject('Object Repository/Virologia/Page_Ingreso de Resultado/input_Est_Sec_Id_BTN_CANCEL'))
		
		WebUI.delay(1)
	}
}

String[] resultFC1 = crud.IngresoLP1Virologia(fechaBDD, fechaBDH, numeroSOL, estudio, muestra, longitud)

println ("El examen primero luego de dar cancelar es:" + resultFC1[0])
println ("El examen primero está liberado o no luego de dar cancelar:" + resultFC1[1])

if(resultFC[1].equals(resultFC1[1])){
	filtroF = true
}else{
	filtroF = false
}
...
if (filtroF) {
    println("Para el Estudio: $estudio, Muestra: $muestra y Solicitud Número: $numeroSOL, El Exámen: $resultFC1[0] se mantuvo con valor: $liberado funciona correctamente, Prueba Correcta")
} else {
