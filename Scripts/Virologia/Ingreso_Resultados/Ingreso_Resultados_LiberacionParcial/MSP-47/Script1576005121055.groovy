...
String EstadoEstudio = IDEstadoEstudio.getValue(1, 2)
...
if(longitud > 0){
	...
	int span = WebUI.executeJavaScript('return $("#span_vLIBERADO_0002").length;', null)
	validado = WebUI.executeJavaScript('return $("#span_vVALIDADO_0002").text();', null)
	if(span > 0 && validado == "SI"){
		filtroF = true
	}
	else{
		filtro = false
...
if (filtroF) {
    println("No puede modificar el valor debido a que el exámen está Validado, Prueba Correcta")
} else {
