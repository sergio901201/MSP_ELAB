...
if(longitud > 0){
	...
	if(!CFinal.isEmpty()){
		CFinal = CFinal.substring(CInicial.length()+1,CFinal.length()-1)
	}
...
println (CFiltro.length() + " " + CFinal.length())
...
if (filtroF) {
    println("Se agregó el Filtro Rótulo al Comentario, funciona correctamente, Prueba Correcta")
} else {
