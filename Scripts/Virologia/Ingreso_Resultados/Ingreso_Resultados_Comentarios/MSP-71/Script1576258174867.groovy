...
String rotulo, rotuloF, CInicial, CFinal, CFiltro
if(longitud > 0){
	...
	CInicial = WebUI.executeJavaScript('return $("#gxp0_ifrm").contents().find("#vSOLICITUDCOMENTARIODESCRIPCION").val();', null)
	println ("El valor del Comentario inicial es:" + CInicial)
	
	CFiltro = WebUI.executeJavaScript('return $("#gxp0_ifrm").contents().find("#span_W0008INF_TEXTO_0001").text();', null)
	println ("El valor del Comentario para agregar es:" + CFiltro)
	
	//Click en input de tipo: checkbox, el 1er elemento
	WebUI.executeJavaScript('$("#gxp0_ifrm").contents().find("#W0008vSELECT_0001").click()', null)
	WebUI.delay(1)
	
	//Click en boton Cargar
	WebUI.executeJavaScript('$("#gxp0_ifrm").contents().find("#W0008BUTTON1").click();', null)
	WebUI.delay(1)
	
	//Click en boton Borrar
	WebUI.executeJavaScript('$("#gxp0_ifrm").contents().find("#W0008BTN_CANCEL").click();', null)
	WebUI.delay(1)
	
	WebUI.delay(1)
	CFinal = WebUI.executeJavaScript('return $("#gxp0_ifrm").contents().find("#vSOLICITUDCOMENTARIODESCRIPCION").val();', null)
	println ("El valor del Comentario Final es:" + CFinal)
	
	if(!CFinal.isEmpty()){
		
	}
	println ("El valor del Comentario Final luego de aplicar SubString() y Trim() es:" + CFinal)
}

boolean filtroF

println (CInicial.length() + " " + CFinal.length())
...
if (filtroF) {
    println("No se agregó el Filtro Rótulo al Comentario, funciona correctamente, Prueba Correcta")
} else {
