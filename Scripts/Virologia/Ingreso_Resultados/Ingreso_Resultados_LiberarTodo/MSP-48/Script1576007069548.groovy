...
String[] resultFC = crud.IngresoLT1Virologia(fechaBDD, fechaBDH, numeroSOL, estudio, muestra, longitud)

boolean filtroF, liberado
String valSelect = ""
		
WebUI.executeJavaScript('$("#BTNLIB").click();', null)

WebUI.delay(1)

/*WebUI.executeJavaScript('$("#TABLE1.Table tbody tr td input#BUTTON2.Button").click();', null)*/
WebUI.click(findTestObject('Object Repository/Virologia/Page_Ingreso de Resultado/input_Confirma liberar todos los exmenes_BUTTON2'))
		
WebUI.delay(2)

String[] resultFC1 = crud.IngresoLT1Virologia(fechaBDD, fechaBDH, numeroSOL, estudio, muestra, longitud)
String[] aux, aux1 = ""

WebUI.delay(1)

//resultFC contiene los examenes antes de dar click en el boton liberar todos/Cancelar
//resultFC1 contiene los examenes despues de dar click en el boton liberar todos/Cancelar
//Recorrer resultFC y resultFC1 y comparar sus valores. Si todos son iguales todo bien, si no todo mal
for(int i=0;i<resultFC.length;i++){
	aux = resultFC[i].split(":")
	aux1 = resultFC1[i].split(":")
	println ("!!!El examen $i para el arreglo resultFC es:" + aux[0] + " y El valor si está Liberado o no es:" + aux[1])
	println ("!!!El examen $i para el arreglo resultFC1 es:" + aux1[0] + " y El valor si está Liberado o no es:" + aux1[1])
	if(aux[0].equals(aux1[0]) && aux[1].equals(aux1[1])){
		filtroF = true
	}
	else{
		filtroF = false
...
if (filtroF) {
    println("Para el Estudio: $estudio, Muestra: $muestra y Solicitud Número: $numeroSOL, Todos los Exámen se mantuvieron con su valor, funciona correctamente, Prueba Correcta")
} else {
