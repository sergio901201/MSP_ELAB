...
//Select Solo Muestras Compartidas
WebUI.executeJavaScript('$("#vSOLOCOMPARTIDAS").click();', null)

boolean smc = WebUI.executeJavaScript('return $("#vSOLOCOMPARTIDAS").prop("checked");', null)
println ("El valor de la booleana es:" + smc)

//Verify Activo Solo Muestras Compartidas
WebUI.verifyElementChecked(findTestObject('Object Repository/Virologia/Page_Bienvenida/Page_Bandeja de trabajo de Unidades (1)/input_Paciente_vSOLOCOMPARTIDAS'),30)
...
CRUD crud = new CRUD()
String[] resultFC = crud.FiltroSMCVirologia(fechaBDD, fechaBDH, smc)
...
if (filtroF) {
    println('El filtro Solo Muestras Compartidas funciona correctamente, Prueba Correcta')
} else {
